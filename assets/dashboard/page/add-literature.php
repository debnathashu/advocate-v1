		...
		if($FIRealName){
			if (is_uploaded_file($FIFileName)){
				if(move_uploaded_file($FIFileName, "assets/img/literature/$FIRealName")) { 
					$sql = $connection->query("INSERT INTO literature (name, author, date, image, post)Values('{$Name}', '{$Author}', '{$Date}', '{$FIRealName}', '{$Post}')");// value insert into database
				}						
			}
		}else{
			$sql = $connection->query("INSERT INTO literature (name, author, date, post)Values('{$Name}', '{$Author}', '{$Date}', '{$Post}')");// value insert into database
		}
	}
?>
<head>
	<?php include"assets/include/tag.php"; ?>
	<title>Add Literature | Adv Mahbub Alam | Lockers IT Solution</title>
	<!-- Bootstrap -->
		...
		<div class="row">
			<div class="col-md-8 col-sm-8 col-md-offset-2 col-sm-offset-2 margin30 sky-form-columns contact-sky-option">
				<form action="" method="post" id="sky-form" class="sky-form sky-form-columns">
					<fieldset>	
						<p class="margin30">Add New Literature on your Tree.</p>
						<section>
						...
						<section>
							...
							<label class="textarea">
								...
								<textarea rows="4" name="post" id="post"></textarea>
							</label>
						</section>
					</fieldset>
					<footer>
					...
					<div class="message">
						...
						<p>New Literature successfully Added.</p>
