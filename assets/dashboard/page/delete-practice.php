...
<?php
	if(isset($_POST['del'])){
		$ID = $_POST['confirm'];
		$sql = $connection->query("delete from parea where id='$ID'");
		header('Location: dashboard.php?page=view-practice');
	}
?>
<head>
	<?php include"assets/include/tag.php"; ?>
	<title>Delete Practice Area | Adv Mahbub Alam | Lockers IT Solution</title>
	<!-- Bootstrap -->
					...
					<fieldset>	
						<section>
							<div class="team-wrap">
								...
								<input type="checkbox" name="confirm" id="confirm" value="<?php echo $row_parea["id"]; ?>" required >Yes! I want to delect.
							</div><!--team-wrap-->
						</section>
					</fieldset>
					<footer>
						<a href="dashboard.php?page=view-practice" class="btn btn-theme-bg btn-ico btn-lg"><i class="fa fa-reply"></i> Cancel</a>
						<button type="submit" name="del" id="del" class="btn btn-theme-bg btn-lg pull-right">Delete it!  <i class="fa fa-trash"></i></button>
					</footer>
					<div class="message">
						<i class="fa fa-check"></i>
						<p>Practice delete successfully.</p>
						<p><a href="dashboard.php?page=view-practice">Back to view all Practice Area.</a></p>
					</div>
				</form>					
			</div>
		</div>
