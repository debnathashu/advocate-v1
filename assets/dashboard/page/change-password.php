<?php
	$user = $connection->query("select * from user where id='$UI'");
...
<?php
	...
	if(isset($_POST['update'])){
		...
		$CPW = $_POST['cpw'];
		if($NPW == $CPW){
			$sql = $connection->query("UPDATE user set password='{$NPW}' where id='$UI'");// value insert into database
		}
	}
?>
<head>
	<?php include"assets/include/tag.php"; ?>
	<title>Change Password | Adv Mahbub Alam | Lockers IT Solution</title>
	<!-- Bootstrap -->
		...
		<div class="row">
			<div class="col-md-8 col-sm-8 col-md-offset-2 col-sm-offset-2 margin30 sky-form-columns contact-sky-option">
				<form action="" method="post" id="sky-form" class="sky-form sky-form-columns">
					<fieldset>	
						<p class="margin30">Update Your Account Password</p>
						<p class="margin30"><font color="red">Recent Password:</font> <a href="#"><?php echo $row_user['password']; ?></a></p>
						<section>
					...
					<footer>
						...
						<button type="submit" name="update" id="update" class="btn btn-theme-bg btn-lg pull-right">Change Password</button>
					</footer>
					<div class="message">
						<i class="fa fa-check"></i>
						<p>Password successfully Changed.</p>
						<p><a href="dashboard.php?page=change-password">Go To Password.</a></p>
					</div>
				</form>					
			</div>
		</div>
