...
<?php
	if(isset($_POST['del'])){
		$ID = $_POST['confirm'];
		$sql = $connection->query("delete from attorneys where id='$ID'");
		header('Location: dashboard.php?page=view-attorneys');
	}
?>
<head>
	<?php include"assets/include/tag.php"; ?>
	<title>Delete Attorneys | Adv Mahbub Alam | Lockers IT Solution</title>
	<!-- Bootstrap -->
			...
			<div class="col-md-8 col-sm-8 col-md-offset-2 col-sm-offset-2 margin30 sky-form-columns contact-sky-option">
				<form action="" method="post" id="sky-form" class="sky-form sky-form-columns">
					<fieldset>	
						<section>
							<div class="team-wrap">
								<h2><?php echo $row_attorneys['name']; ?></h2>
								...
								<input type="checkbox" name="confirm" id="confirm" value="<?php echo $row_attorneys["id"]; ?>" required >Yes! I want to delect <?php echo $row_attorneys["name"]; ?>.
							</div><!--team-wrap-->
						</section>
					</fieldset>
					<footer>
						<a href="dashboard.php?page=view-attorneys" class="btn btn-theme-bg btn-ico btn-lg"><i class="fa fa-reply"></i> Cancel</a>
						<button type="submit" name="del" id="del" class="btn btn-theme-bg btn-lg pull-right">Delete it!  <i class="fa fa-trash"></i></button>
					</footer>
					<div class="message">
						<i class="fa fa-check"></i>
						<p>Attorneys delete successfully.</p>
